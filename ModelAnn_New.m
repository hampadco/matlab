...
for i_1=[2,3,5,10,15,20]
    ...
    for i_2=[2,3,5,10,15,20]
        ...
        %%
        Target_Train(isnan(Target_Train)==1)=mean(Target_Train,'omitnan');
        Target_Test(isnan(Target_Test)==1)=mean(Target_Test,'omitnan');
        ...
        Score_Train_temp = Y_Train_ANN;
        Score_Test_temp = Y_test_ANN;
        ...
        if curr_precision > precision_best
           precision_best = curr_precision;
           net_best = net;
           Y_Train_ANN_best = Y_Train_ANN;
           Y_test_ANN_best = Y_test_ANN;
           Score_Train_best = Score_Train_temp;
           Score_Test_best = Score_Test_temp;
...
%% Evaluate Train
[confMat, ~] = confusionmat(Target_Train_best, Y_Train_ANN_best);
...
%% Evaluate Test
[confMat, ~] = confusionmat(Target_Test_best, Y_test_ANN_best);
TP_Test = confMat(2,2);
FP_Test = confMat(1,2);
TN_Test = confMat(1,1);
FN_Test = confMat(2,1);
...
%% Plot ROC
[X1,Y1,~,AUC_Train] = perfcurve(Target_Train_best, Score_Train_best, 1);
[X2,Y2,~,AUC_Test] = perfcurve(Target_Test_best, Score_Test_best, 1);
[X,Y,~,AUC_all] = perfcurve([Target_Train_best;Target_Test_best], [Score_Train_best;Score_Test_best], 1);
